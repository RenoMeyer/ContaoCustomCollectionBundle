<?php

// Backend Modules
$GLOBALS['BE_MOD']['content']['ccol'] = [
    'tables' => ['tl_custom_collection_archive', 'tl_custom_collection'],
    'icon' => 'data:image/png;base64,',
];

// Frontend modules
$GLOBALS['FE_MOD']['miscellaneous']['ccol'] = 'Oneup\Bundle\ContaoCustomCollectionBundle\Module\CustomCollectionModule';

// Models
$GLOBALS['TL_MODELS']['tl_custom_collection'] = 'Oneup\Bundle\ContaoCustomCollectionBundle\Model\CustomCollectionModel';
$GLOBALS['TL_MODELS']['tl_custom_collection_archive'] = 'Oneup\Bundle\ContaoCustomCollectionBundle\Model\CustomCollectionArchiveModel';
